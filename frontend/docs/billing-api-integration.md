...
| 功能 | 方法 | 路径 | 请求体/参数 | 响应 |
| --- | --- | --- | --- | --- |
| 列表模板 | GET | `/api/v1/billing/templates` | query：`templateType`（GLOBAL/GROUP/CUSTOMER，必选）、`keyword`（按名称/编码）、`status`、`customerId`、`customerGroupId`、`limit/offset` | `items: Template[]`、`total` |
...
| 启用/停用 | PATCH | `/api/v1/billing/templates/{id}/status` | body：`{ status: 'ACTIVE' | 'INACTIVE' | 'DRAFT' }` | 200/204 |

> 说明：前端当前未内置“删除确认”以外的工作流，若需要审核/版本化，可在后端扩展 `status=REVIEWING` 或新增版本接口。
...
## 前端对接点
- 列表页：根据路由 tab 请求 `templateType` 对应的列表；支持 keyword/status 过滤；显示 total。
...
- 状态切换：调用 `PATCH /status`；前端无需额外校验，后端阻断不合法状态变更。
- 删除：调用 DELETE。
